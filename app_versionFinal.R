...
################################################################################### server Hyt part
  ...
  output$main_tyt <- renderPlot({
    ...
    if(input$ViewTypeAtyt=="offline"){
      ...
      # g1
      redeemtime(range=input$ViewRangeAtyt,type="offline")
    }else{
      # g2 <- redeemtime(range=input$ViewRangeAtyt,type="online")
      # g2
      redeemtime(range=input$ViewRangeAtyt,type="online")
    }
  })
  output$column_tyt <- renderPlot({
    ...
    # g3
    UseRateCouponRatecol(discount_col = input$ViewDiscountcoltyt,type=input$ViewTypeBtyt)
  })
  output$boxplot_tyt <- renderPlot({
    print('ok')
    # g4 <- UseRateCouponRatebox(discount_box = input$ViewDiscountboxtyt1,type=input$ViewTypeBtyt)
    # g4
    UseRateCouponRatebox(discount_box = input$ViewDiscountboxtyt1,type=input$ViewTypeBtyt)
  })
  #TYT server part end
  # #######################################################################################TYT server part end
